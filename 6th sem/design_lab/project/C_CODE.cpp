...
{ 
    ...
    for(int i=0;i<n/6;i++)
    {
      ...
      //  him[i]=-him[i];
        xnewre[i]=(yre[i]*hre[i])-(yim[i]*him[i]);
        xnewim[i]=(yre[i]*him[i])+(hre[i]*yim[i]);
                
        norm=hre[i]*hre[i]+(him[i]*him[i]);
        
        xnewre[i]=(xnewre[i]/norm)*(2^8);
        xnewim[i]=(xnewim[i]/norm)*(2^8);
    }
...
for(i=0;i<n/6;i++)
{
    re[i]=(hre[i]*xnewre[i])-(him[i]*xnewim[i])+(vre[i]*a);
    im[i]=(hre[i]*xnewim[i])+(him[i]*xnewre[i])+(vim[i]*a);
    norm=(hre[i]*hre[i])+(him[i]*him[i]);
